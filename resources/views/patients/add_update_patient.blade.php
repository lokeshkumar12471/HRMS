						...
						<div class="widget-area-2 proclinic-box-shadow">
                           ...
                           @else
							<h3 class="widget-title">Add Patient</h3>
                             @endif
							<form  @if (isset($patient->id)!='') action="{{ route('patientupdate',$patient->id) }}" @else action="{{ route('patientstore') }}" @endif method="post" enctype="multipart/form-data">
                                @csrf
								<div class="form-row">
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        @if($patient->id!='') <option
                                       value="{{ $doctor->id }}"style="background-color:grey">{{ $doctor->doctor_name }}  </option> @endif
                                   ...
                                   placeholder="Date of Birth" class="form-control" id="dob">
									...
									<div class="form-group col-md-12">
                                        ...
                                        @if (isset($patient->id) != '')
  placeholder="Address" class="form-control" id="exampleFormControlTextarea1" rows="3">{{ $patient->patient_address }}</textarea>@endif
									</div>
									<div class="form-group col-md-12">
										<label for="file">File</label>
										<input type="file"  name="patient_profile" class="form-control" id="file">
                                        @if(isset($patient->id)!="") <img src="{{ asset('uploads/patient/'. $patient->patient_profile) }}"  name="patient_profile" width="100" heigth="100">@endif
									</div>
...
$('#dept').change(function () {
 ...
 $(".removedata").remove();
            $.ajax({
                ...
                {
               $(".doctors").append("<option value='"+items.id+"' class='removedata'>"+items.doctor_name+"</option>");
                }
                }
